...
class MainnetDeploymentHelper {
  ...
  async connectCoreContractsMainnet(contracts, LQTYContracts, chainlinkProxyAddress) {
    ...
    await this.isOwnershipRenounced(contracts.stabilityPool) ||
      await this.sendAndWaitForTransaction(contracts.stabilityPool.setAllowFrontEndKickback(
        false,
{ gasPrice }
    ));
  ...
  async connectUnipoolMainnet(uniPool, LQTYContracts, LUSDWETHPairAddr, duration) {
    ...
    await this.isOwnershipRenounced(uniPool) ||
      await this.sendAndWaitForTransaction(uniPool.setParams(LQTYContracts.lqtyToken.address, LUSDWETHPairAddr, LQTYContracts.lqtyStaking.address, duration, {gasPrice}))
    console.log(`Unipool reward duration ${duration} secs, ${duration / 3600 / 24} days`)
  }
