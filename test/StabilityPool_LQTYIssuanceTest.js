...
contract('StabilityPool - LQTY Rewards', async accounts => {
  ...
  const calculateWithdrawableLQTYRewards = async (address) => {
    const withdrawable = await lqtyStaking.withdrawableEarning(address)
    return withdrawable[0].add(withdrawable[1])
  }
  ...
  describe("LQTY Rewards", async () => {
    ...
    beforeEach(async () => {
      ...
      await contracts.lusdToken.unpause();
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct LQTY gain. No liquidations. No front end.", async () => {
      ...
      for (addr of [A, B, C]) {
        assert.isAtMost(getDifference((await calculateWithdrawableLQTYRewards(addr)), expectedLQTYGain_2yr), 1e12)
      }
    })
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct LQTY gain. No liquidations. No front end.", async () => {
      ...
      // Check LQTY balances increase by correct amount
      for ([addr, expected] of [[A, A_expectedLQTYGain_2yr], [B, B_expectedLQTYGain_2yr], [C, C_expectedLQTYGain_2yr]]) {
        assert.isAtMost(getDifference((await calculateWithdrawableLQTYRewards(addr)), expected), 1e12)
      }
    })

    // A, B, C deposit. Varied stake. 1 Liquidation. D joins.
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct LQTY gain. No liquidations. No front end. D comes.", async () => {
      const initialIssuance = await communityIssuanceTester.totalLQTYIssued()
      assert.equal(initialIssuance, 0)
      ...
      // Check LQTY balances increase by correct amount
      for ([addr, expected] of [[A, A_expectedTotalGain], [B, B_expectedTotalGain], [C, C_expectedTotalGain], [D, D_expectedTotalGain]]) {
        assert.isAtMost(getDifference((await calculateWithdrawableLQTYRewards(addr)), expected), 1e12)
      }
    })
    ...
    it('withdrawFromSP(): Depositor withdraws correct LQTY gain after serial pool-emptying liquidations. No front-ends.', async () => {
      ...
      for (depositor of [A, B]) {
        const LQTYBalance = await calculateWithdrawableLQTYRewards(depositor)
    ...
    it('LQTY issuance for a given period is not obtainable if the SP was empty during the period', async () => {
      ...
      // Get LQTY Gains
      const A_LQTYGain = await calculateWithdrawableLQTYRewards(A)
      const C_LQTYGain = await calculateWithdrawableLQTYRewards(C)
    ...
    it("withdrawFromSP(): Several deposits of 100 LUSD span one scale factor change. Depositors withdraw correct LQTY gains", async () => {
      ...
      const LQTYGain_A = await calculateWithdrawableLQTYRewards(A)
      const LQTYGain_B = await calculateWithdrawableLQTYRewards(B)
      const LQTYGain_C = await calculateWithdrawableLQTYRewards(C)
      const LQTYGain_D = await calculateWithdrawableLQTYRewards(D)
      const LQTYGain_E = await calculateWithdrawableLQTYRewards(E)
      const LQTYGain_F = await calculateWithdrawableLQTYRewards(F)
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct LQTY gain. No liquidations. Front ends and kickback rates.", async () => {
      ...
      // Check LQTY balances increase by correct amount
      assert.isAtMost(getDifference((await calculateWithdrawableLQTYRewards(A)), A_expectedFinalGain), 1e12)
      assert.isAtMost(getDifference((await calculateWithdrawableLQTYRewards(B)), B_expectedFinalGain), 1e12)
      assert.isAtMost(getDifference((await calculateWithdrawableLQTYRewards(C)), C_expectedFinalGain), 1e12)
      assert.isAtMost(getDifference((await calculateWithdrawableLQTYRewards(D)), D_expectedFinalGain), 1e12)
      assert.isAtMost(getDifference((await calculateWithdrawableLQTYRewards(frontEnd_1)), F1_expectedFinalGain), 1e12)
      assert.isAtMost(getDifference((await calculateWithdrawableLQTYRewards(frontEnd_2)), F2_expectedFinalGain), 1e12)
    })
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct LQTY gain. Front ends and kickback rates", async () => {
      ...
      const F1_LQTYBalance_After_M2 = await calculateWithdrawableLQTYRewards(frontEnd_1)
      ...
      const B_LQTYBalance_After_M3 = await await calculateWithdrawableLQTYRewards(B)
      ...
      const F1_LQTYBalance_After_M3 = await calculateWithdrawableLQTYRewards(frontEnd_1)
      ...
      const F2_LQTYBalance_After_M3 = await calculateWithdrawableLQTYRewards(frontEnd_2)
      ...
      const A_FinalLQTYBalance = await calculateWithdrawableLQTYRewards(A)
      const B_FinalLQTYBalance = await calculateWithdrawableLQTYRewards(B)
      const C_FinalLQTYBalance = await calculateWithdrawableLQTYRewards(C)
      const D_FinalLQTYBalance = await calculateWithdrawableLQTYRewards(D)
      const E_FinalLQTYBalance = await calculateWithdrawableLQTYRewards(E)
      const F1_FinalLQTYBalance = await calculateWithdrawableLQTYRewards(frontEnd_1)
      const F2_FinalLQTYBalance = await calculateWithdrawableLQTYRewards(frontEnd_2)
    ...
    it("withdrawFromSP(): Several deposits of 10k LUSD span one scale factor change. Depositors withdraw correct LQTY gains", async () => {
      ...
      const LQTYGain_F1 = await calculateWithdrawableLQTYRewards(frontEnd_1)
